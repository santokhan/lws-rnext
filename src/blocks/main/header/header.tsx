...
const Header: React.FC<HeaderProps> = ({ onFilter, onSorting }) => {
    function onSubmit(e: FormEvent<HTMLFormElement>) {
        e.preventDefault();
        const formData = new FormData(e.currentTarget);
        const needle: FormDataEntryValue | null = formData.get("search_dropdown");
        //  && needle.length > 0
        if (typeof needle === 'string') {
            onFilter(needle.trim());
        }
    }
    ...
    return (
        <header className="mb-8 lg:mb-10 mx-auto max-w-7xl px-4">
